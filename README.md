...
<td class="center">视频 / 图片 / 文本 / Cookie增强</td>
</tr>
<tr>
...
<td class="center">视频 / 图片 / 文本 / 热评 / Cookie增强</td>
</tr>
<tr>
...
| 功能 | 说明 |
|------|------|
| 解析直播间 | 获取直播间信息，显示直播状态和观看人数 |
| 解析专栏 | 获取专栏文章内容和图片 |
| 解析收藏夹 | 获取收藏夹内视频列表和封面（需要Cookie） |
...
为小红书和抖音配置 Cookie 后，可以获取更完整的媒体内容和更好的解析成功率。

### 小红书 Cookie 配置

1. 在 `小红书增强 → 使用Cookie解析` 中开启
2. 填入小红书 Cookie（浏览器登录后 F12 → Network → 任意请求的 Cookie 头）
3. Cookie 可以帮助获取更完整的内容信息和更高的解析成功率

### 抖音 Cookie 配置

1. 在 `抖音增强 → 使用Cookie解析` 中开启
2. 填入抖音 Cookie（浏览器登录后 F12 → Network → 任意请求的 Cookie 头）
3. Cookie 可以帮助获取更完整的视频信息和更高的解析成功率

> **注意**：Cookie 配置为可选项，不配置时插件会使用无 Cookie 方式解析
